 ...
 * If there are no free frames remaining, then the LFU
    ...
    */
    ...
    public static int do_handlePageFault(ThreadCB thread, 
    	...
    	if (freeFrame != null) {
    		...
    		availableFrame = PageFaultHandler.performLFU();
    		availableFrame.setReserved(task);
    		...
    		//reset the reference count for the frame back to 0
    		MMU.referenceCount[availableFrame.getID()] = 0;
    ...
    /**
     * Perform a LFU page replacement algorithm to evict a frame
     * for the invalid page.  The reference counts for the frames 
     * are maintained in an array initialized in MMU.  The algorithm
     * returns a clean least referenced frame.  If all frames are dirty,
     * the least referenced one is returned. 
     * 
     * @return a frame that will be used for the invalid page
     */
    public static FrameTableEntry performLFU() {
    	ArrayList<FrameTableEntry> cleanFrames = new ArrayList<FrameTableEntry>();
    	ArrayList<FrameTableEntry> availableFrames = new ArrayList<FrameTableEntry>();
    	
    	for (int i = 0; i < NUMBER_OF_FRAMES; i++) {
    		FrameTableEntry frame = MMU.getFrame(i);
    		//locked and reserved frames cannot be selected
    		if (frame.getLockCount() > 0 || frame.isReserved()) {
    			continue;
    		} 
    		...
    		//check if there are any clean frames
    		if (!frame.isDirty()) {
    			cleanFrames.add(frame);
    		}
    	}
    	
    	int frameID, leastReferenceCount;
    	//if there are clean frames, return the least referenced one
    	if (cleanFrames.size() > 0) {
    		frameID = cleanFrames.get(0).getID();
    		leastReferenceCount = MMU.referenceCount[frameID];
        	
        	for (int i = 0; i < cleanFrames.size(); i++) {
        		int id = cleanFrames.get(i).getID();
        		if (MMU.referenceCount[id] < leastReferenceCount) {
        			leastReferenceCount = MMU.referenceCount[id];
        			frameID = id;
        		}
        	} 
        	
        	return MMU.getFrame(frameID);
    	} else { //no clean frames
    		frameID = availableFrames.get(0).getID();
    		leastReferenceCount = MMU.referenceCount[frameID];

    		for (int i = 0; i < availableFrames.size(); i++) {
    			int id = availableFrames.get(i).getID();
    			if (MMU.referenceCount[id] < leastReferenceCount) {
    				leastReferenceCount = MMU.referenceCount[id];
    				frameID = id;
    			}
